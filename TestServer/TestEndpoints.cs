...
namespace TestServer.IntegrationTests;

/// <summary>
/// Integration tests for endpoints - testing the full HTTP pipeline
/// </summary>
[Collection("Sequential")]
public class EndpointIntegrationTests : IClassFixture<TestWebApplicationFactory<Program>>
{
    private readonly TestWebApplicationFactory<Program> _factory;
    private static bool _databaseInitialized;
    private static readonly object _lock = new object();

    public EndpointIntegrationTests(TestWebApplicationFactory<Program> factory)
    {
        _factory = factory;
        lock (_lock)
        {
    ...
    [Fact]
    public async Task Get_UserEndpoint_ShouldReturnUserProfile()
    {
        using var scope = _factory.Services.CreateScope();
    ...
    [Fact]
    public async Task Patch_UserEndpoint_ShouldUpdateUserProfile()
    {
        var client = _factory.CreateClient();
    ...
    [Fact]
    public async Task Patch_UserEndpoint_ShouldReturnBadRequest_WhenLanguageLevelIsInvalid()
    {
    ...
    [Fact]
    public async Task Get_NonExistentUser_ShouldReturnNotFound()
    {
        var client = _factory.CreateClient();
    ...
    #region Learning Endpoint Tests

    [Fact]
    public async Task Get_PracticeNouns_ShouldReturnNounsForUserLevel()
    {
        var client = _factory.CreateClient();
    ...
    [Fact]
    public async Task Get_LearningProgress_ShouldReturnProgressData()
    {
        var client = _factory.CreateClient();
    ...
    [Fact]
    public async Task Delete_AllProgress_ShouldReturnNoContent()
    {
        var client = _factory.CreateClient();
        ...
        using var scope = _factory.Services.CreateScope();
        var service = scope.ServiceProvider.GetRequiredService<IStorageService>();
        await service.UpsertNounProgressAsync(1, 1, true);
        
        // Then delete all progress
    ...
    #region Spaced Repetition Algorithm Tests

    [Fact]
    public async Task SpacedRepetition_ShouldPrioritizeNounsWithLowerTimeFrame()
    {
        using var scope = _factory.Services.CreateScope();
    ...
    [Fact]
    public async Task SpacedRepetition_ShouldHandleBoundaryConditions()
    {
        using var scope = _factory.Services.CreateScope();
